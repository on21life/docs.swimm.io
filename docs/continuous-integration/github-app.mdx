...
**Swimm Verify Check Run**

You can rerun an existing Swimm Verify check run via the Swimm GitHub App in cases, for example, when a check run is canceled or if there is an issue with the initial check run.

**Timeouts for Swimm Verify Checks**

A timeout is available for Swimm Verify checks to let users re-run a check which did not finish due to an internal issue. 
...
<Link href="https://swimm.io/blog/soc-2-compliance-a-guide-for-developers/">Swimm is SOC2 compliant</Link> and ISO 27001 certified. Read about security aspects of our GitHub app <Link href="https://docs.swimm.io/security/">here.</Link><br/> 
