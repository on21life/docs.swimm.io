...
Swimm’s GitHub App has the option to run Swimm Verify only on files that are changed in the PR.
...
**Timeouts for Swimm Verify Checks** - A timeout is available for Swimm Verify checks to let users re-run a check which did not finish due to an internal issue.
...
<br />
Reach out to us at <u>info@swimm.io</u> to request Swimm’s SOC 2 Type II report.
